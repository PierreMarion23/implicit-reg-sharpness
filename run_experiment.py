...
def make_plots_full_experiment(
    ...
    plt.savefig(
        "figures/divergence_probability_{}.png".format(name),
        bbox_inches="tight",
    )
...
def run_sub_experiment(
    ...
    for step_size, scale, num_steps, palette_idx in zip(
        ...
        axs[0].plot(
            ...
            * jnp.arange(len(train_losses[::compute_sharpness_every_step])),
            train_losses[::compute_sharpness_every_step],
            color=palette[palette_idx],
        )
        axs[0].plot(
            compute_sharpness_every_step
            * jnp.arange(len(test_losses[::compute_sharpness_every_step])),
            test_losses[::compute_sharpness_every_step],
            color="green",
        )
